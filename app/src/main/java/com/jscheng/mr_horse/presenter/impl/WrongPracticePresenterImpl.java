...
public class WrongPracticePresenterImpl implements PracticePresenter {
    ...
    private void initStoreData() {
        ...
        loadSubscription = Observable.create(new Observable.OnSubscribe<List<QuestionModel>>() {
            ...
            public void call(Subscriber<? super List<QuestionModel>> subscriber) {
                temQuestionList = QuestionModelLoadUtil.getWrongQuestionModel(mContext,catogory);
    ...
    public void onPageSelected(int position) {
        ...
        QuestionModelLoadUtil.saveQuetionWrongDoneId(mContext, catogory, questionModelList.get(position).getQuestionNum());
        mPracticeView.showPageNumView(pageNum+1,questionModelList.size());
        showCollect();
    }
    ...
    private void removeWrong(QuestionModel model){
        ...
        QuestionModelLoadUtil.setQuestionModelOutWrong(mContext,model);
        ...
        mPracticeView.changePaperView(pageNum,false,0);
        mPracticeView.showPageNumView(pageNum+1,questionModelList.size());
        QuestionModelLoadUtil.saveQuetionWrongDoneId(mContext, catogory, questionModelList.get(pageNum).getQuestionNum());
        showCollect();
    }
    ...
    private void getPageNumFromDb() {
        ...
        if (this.pageNum >= 0)
            return;
        int questionId = QuestionModelLoadUtil.getQuestionWrongDoneId(mContext,catogory);
